                ...
                >
                    <CarouselContent className=''>
                        {events.map((event, index) => (
                            ...
                            <CarouselItem 
                                key={index} 
                                className={cn('max-h-[448px] max-w-[728px] basis-1/3 overflow-visible', (index === 0 ? selectedIndex === events.length - 1 : selectedIndex === index - 1) ? 'z-[9999] cursor-pointer' : 'blur-sm mt-14')}
                                onClick={() => {
                                    if(index === 0 && events.length - 1 === selectedIndex) router.push(`/events/${event.id}`)
                                    else if(index - 1 === selectedIndex) router.push(`/events/${event.id}`)
                                    api?.scrollTo(index === 0 ? events.length - 1 : index - 1)
                        ...
                        <>
                            ...
                            <div className='flex gap-4 items-center justify-between flex-1 w-full my-6'>
                                <div className='flex gap-2 flex-1'>
                                    ...
                                    <div className='flex flex-col gap-2 justify-between items-end text-nowrap h-36 w-full'>
                                        <p className='font-poppins font-extralight text-base text-left w-full'>{selectedIndex === events.length - 1 ? events[0].venue : events[selectedIndex + 1].venue}</p>
                                        <p className='font-poppins font-extralight text-base text-left w-full'>{selectedIndex === events.length - 1 ? `${months[events[0].eventDate?.getMonth()]}, ${getDaySuffix(events[0].eventDate.getDate())}, ${events[0].eventDate.getFullYear()}` : `${months[events[selectedIndex + 1].eventDate?.getMonth()]}, ${getDaySuffix(events[selectedIndex + 1].eventDate.getDate())}, ${events[selectedIndex + 1].eventDate.getFullYear()}`}</p>
                                        <p className='font-poppins font-extralight text-base text-left w-full'>{selectedIndex === events.length - 1 ? events[0].country : events[selectedIndex + 1].country},{selectedIndex === events.length - 1 ? events[0].city : events[selectedIndex + 1].city}</p>
                                    </div>
                                </div>
                                <div className='flex flex-col gap-2 lg:gap-6 items-end w-full h-full'>
