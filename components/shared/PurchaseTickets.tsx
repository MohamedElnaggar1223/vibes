...
export default function PurchaseTickets({ event, exchangeRate, user, locale }: Props) {
    ...
    useEffect(() => {
        if (!foundEvent) addEvent({ eventId: event.id, selectedTickets: availableTickets.reduce((acc, ticket) => ({ ...acc, [ticket.name]: 0 }), {} as { [x: string]: number }), selectedSeats: {}, confirmedSeats: {}, purchasedTickets: availableTickets.reduce((acc, ticket) => ({ ...acc, [ticket.name]: 0 }), {} as { [x: string]: number }), purchasedParkingPass: 0 })
    }, [])
    ...
    const handleBuyTickets = async () => {
        ...
        try {
            ...
            if (eventData.seated) {
                ...
                console.log(foundEvent.purchasedTickets)
    ...
    return (
        <AnimatePresence>
            <div className='relative flex-1 flex flex-col py-2 px-2 gap-6 max-lg:w-full max-lg:min-h-[80vh] max-lg:mb-4'>
                ...
                {
                    Object.values(foundEvent.purchasedTickets).length > 0 ? (
                        <div ref={parentRef} className='flex-1'>
                            <div className='h-full overflow-auto py-2' style={{ maxHeight: `${maxHeight}px` }}>
                                {Object.keys(foundEvent.purchasedTickets).slice().map((ticket) => (
