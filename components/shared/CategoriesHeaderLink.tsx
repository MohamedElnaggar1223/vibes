...
export default function CategoriesHeaderLink({ categories }: Props) {
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger>
                <p className={cn('font-poppins max-md:py-2 text-lg font-[300] z-[9999]', pathname?.includes('/categories') ? 'bg-[linear-gradient(90deg,rgba(231,35,119,1)50%,rgba(235,94,27,1)100%)] text-transparent bg-clip-text' : 'text-white')}>{t('categories')}</p>
            </PopoverTrigger>
